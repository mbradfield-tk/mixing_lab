...
text report — no Streamlit, no pandas required. Run from the project root::
...
def build_report(con: sqlite3.Connection, *, days: int | None,
    ...
    unique_sessions = con.execute(
        "SELECT COUNT(DISTINCT session_id) FROM access_log"
    ).fetchone()[0]
    ...
    print("=" * 52)
    print("  MIXING LAB — USAGE SUMMARY")
    print("=" * 52)
    ...
    ip_rows = con.execute(
        ...
        "COUNT(DISTINCT session_id) AS sessions, MAX(ts_utc) AS last_seen "
        "FROM access_log GROUP BY client_ip ORDER BY n DESC LIMIT ?",
        (top,),
    ).fetchall()

    _hr(f"Top {top} client IP addresses")
    print(f"  {'IP address':<24}{'Hits':>7}{'Sessions':>10}  Last seen")
    for r in ip_rows:
        ip = r["client_ip"] or "(unknown)"
        print(f"  {ip:<24}{r['n']:>7,}{r['sessions']:>10,}  "
              f"{_fmt_local(r['last_seen'])}")

    # Recent activity
    if recent > 0:
        rec_rows = con.execute(
            "SELECT ts_utc, client_ip, page, session_id "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Print a text summary of Mixing Lab usage from usage.db."
    )
    parser.add_argument(
